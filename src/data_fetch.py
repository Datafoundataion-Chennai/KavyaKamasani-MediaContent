...
def get_channel_videos(channel_id):
    ...
    while len(video_ids) < 10000:
        params = {"part": "id", "channelId": channel_id, "maxResults": 50, "order": "date", "type": "video", "pageToken": next_page_token, "key": API_KEY}
...
def get_video_details(video_ids):
    ...
    for i in range(0, len(video_ids), 50):
        ...
        for item in response.get("items", []):
            ...
            videos.append({
                "video_id": item["id"], "title": snippet["title"], "channel_title": snippet["channelTitle"],
                "publish_time": snippet["publishedAt"], "views": stats.get("viewCount", 0), "likes": stats.get("likeCount", 0),
            })
        time.sleep(1)
    return videos
